...
if __name__ == "__main__":
    bag_file_path = '/home/jacob/Documents/the_ros2_ws/src/Crazyflie-P4-Group-460/Data_Processing/bags/Nye tests/z-test-2/z-test-1.0/2025-05-13-09-45-13_0.mcap'
    ...
    z_ref = 1.0  # Use mean height as reference
    ...
    mean_z = np.mean(filtered_positions)
    mean_deviation = np.mean([(z - z_ref) for z in filtered_positions])
    rmse = np.sqrt(np.mean([(z - mean_z) ** 2 for z in filtered_positions]))

    print(f"\nAnalysis Results:")
    print(f"Reference Z height: {z_ref:.4f} meters")
    print(f"Maximum deviation: {max_deviation:.4f} meters")
    print(f"Mean deviation: {mean_deviation:.4f} meters")
    print(f"Mean Z position: {mean_z:.4f} meters")
